...
% Topological Data Analysis (TDA) is a recent field that emerged from various works in applied algebraic topology and computational geometry. The aim of TDA is to extract and analyze relevant topological and geometric features of real-world data, which are often high-dimensional and have complex underlying geometric structures. Our project is motivated by the question: how can high-dimensional point clouds be compared based on their topological structures?

% To this end, we proposed an approach to compare point clouds based on the pWasserstein distance of their topological fingerprints. We illustrated the approach with a concrete application in studying the neural population responses in primate visual system to different visual stimuli. Each neural population response was represented as a highdimensional point cloud. Our approach allowed us to quantify the pairwise similarity of the neural population responses in terms of their topological structures. Using this approach, we also compared the topological structure of each neural population response with that of synthetic data sampled from 2-sphere and torus. Our results showed that in terms of topological structures, the neural response to one specific type of stimuli is significantly different from the others, which suggests that further investigation in neuroscience could
% potentially uncover new understanding of the primate visual system. 
% \end{abstract}
