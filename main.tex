...
\thesistitle{Topology-based Comparison of Neural Population Responses via Persistent Homology and p-Wasserstein Distance} % Your thesis title, this is used in the title and abstract, print it elsewhere with \ttitle
...
Topological Data Analysis (TDA) is a recent field that emerged from applied algebraic topology and computational geometry. The aim of TDA is to analyze the topological structures underlying real-world data that are usually represented as point clouds in some metric space and are often
high-dimensional. 

Neural population activity gives rise to a high-dimensional point cloud. 

Our project is motivated by the question: how can high-dimensional neural population activity be compared based on their topological structures?

To this end, we proposed an approach to compare point clouds based on the $p$-Wasserstein distance of their topological structures. 

dim reduction 
persistent homology 
p-Wasserstein distance 
why? 
* When endowed with the Wasserstein metric, the space of persistence diagrams is complete and separable. This allows us to show that the standard statistical objects such as means, variances, and conditional probabilities are well defined. With this, we can perform statistical inference on persistence diagrams,

We illustrated the approach with a concrete application in studying the high-dimensional neural population responses in the mouse retina to six different visual stimuli (low-frequency gratings, high-frequency gratings, 1-dot negative flows, 3-dot negative flows, 1-dot positive flows, and 3-dot positive flows). Each neural population response was represented as a high-dimensional point cloud. Our approach allowed us to quantify the pairwise similarity of the neural population responses in terms of their topological structures. 

put in persistent homology somewhere! 

Specifically, we investigated the topological structure of each neural population response and compared each against synthetic data sampled from 2-sphere and torus. 

advantage of using our approach!! 

Our results showed that in terms of topological structures, the neural response to low-frequency gratings is significantly different from other types of flow stimuli, which suggests that further investigation in neuroscience could potentially uncover new understanding of the primate visual system.
inform further neuroscientific investigation 
...
\input{chapters/chapter4-distances}
\input{chapters/chapter2-diffmap}
\input{chapters/chapter5-application}
